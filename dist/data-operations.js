 ...
 */
export async function loadDataFromDb(dbPath, requestPath) {
    let res;
 ...
 */
export async function applyOperation(baseData, operation) {
    if (!operation) {
        return baseData;
    }
    ...
    if (operation.type === 'deepmerge' && isObject(baseData) && isObject(operation.data)) {
        return deepmerge(baseData, operation.data);
    }
    if (operation.type === 'append' && isArray(baseData) && isArray(operation.data)) {
        return baseData.concat(operation.data);
    }
    return baseData;
}
